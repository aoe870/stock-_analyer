...
const app = Vue.createApp({
  ...
  computed: {
    ...
    enterpriseRefreshMessage() {
      const refresh = this.stockOverview.data_quality?.last_refresh;
      if (!refresh) return "";
      const summary = refresh.summary || {};
      const errors = Number(summary.metadata_errors || 0);
      const rows = Number(summary.rows || 0);
      if (refresh.status === "failed") return `企业数据刷新失败：${refresh.error || refresh.error_message || "数据源暂不可用"}`;
      if (errors > 0 && rows === 0) return `企业数据刷新未获取到股东高管数据，${errors} 个接口请求失败，请稍后重试或检查 Miana 服务连通性。`;
      return "";
    },
  },
